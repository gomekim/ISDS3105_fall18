...
gather(sn_users, key='kim')
#does the same as above: keeps everything BUT year
```

Use the output from the `gather()` above to plot a line chart including both twitter and facebook data.

```{r}
gather(sn_users, key = 'socialNet', value = 'Mil.Users', twitter, facebook)  %>% ggpolot() +  geom_line(aes(x  ='year' , y =  'Mil.Users', color='socialNet'))
```
...
```{r}
gather(untidyReview, key= 'AttribruteName', value = 'Rating', -reviewId, -reviewStayYear)  # use gather to 'tidy' the untidyReview
#pass the tidy dataset to ggplot() for plotting
dt <- gather(untidyReview, key= 'AttribruteName', value = 'Rating', -reviewId, -reviewStayYear)
gather(untidyReview, key= 'AttribruteName', value = 'Rating', -reviewId, -reviewStayYear) %>%
  ggplot(dt, aes(x=Rating, color = Rating)) +
geom_bar() + facet_wrap(~AttribruteName )
...
gather(iphoneSales, key= 'Quarters', value= 'value', -year) %>% 
  ggplot() +
  geom_line(aes(x= value, y = year))

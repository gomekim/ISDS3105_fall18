...
```{r}
#' HINTS:
 hotelFun<- function(data, title) {
                   ggplot(data= filter(hotels, hotels$hotel== "GrandHotelTaipei"))+
                  
    ggsave(h.jpeg)
     
 }

hotels %>% mutate_at(vars(starts_with('hot')), .funs = funs(mean,hotelFun(hotel,AverageRoomRate)))

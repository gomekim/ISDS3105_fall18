...
```{r}
select(sn_revenue, company, y2015)

```

2. Using `untidyReview`, select only the variables containing review ratings.
```{r}
select(untidyReview, -reviewId, -reviewStayYear)
```
...
```{r}
mutate(beerDt, LiterCap= GallonsCapita * 3.78)
```

2. Using `socialNetwork_users` add a percent sign to the variable percent (use `paste0()`)
```{r}
mutate(socialNetwork_users, percent = paste0(percent,"%") )
```
...
```{r}
filter(titanicDt, Survived== 'No') %>%
group_by(Class)%>%
summarise(AvgClass = mean(n))
```
...
```{r}
flights <- full_join(flights,airlines, by= "carrier")
```
